...
const App = ({ children }) => {
    ...
    useEffect(() => {
        setSelectHabilitado(roles.length > 1);
    ...
    useEffect(() => {
        getMyRoles();
    }, [rol, school, sessionStorage.getItem('actual_school')]);

    const items = [
        getItem(<Link to="/perfil">Perfil</Link>, '1', <UserOutlined />),
        getItem(<Link to="/horarios">Horarios</Link>, '2', <TableOutlined />),
        ...((sessionStorage.getItem('rol') ==='Directivo'&& JSON.stringify(data).includes("Directivo") === true) || (sessionStorage.getItem('rol')==='Preceptor'&& JSON.stringify(data).includes("Preceptor") === true)? [
