...
import { Spin, Table, Select, Input, FloatButton, Drawer, Form, Button, message, Modal, Flex, Tooltip } from "antd";
import { SearchOutlined, EditOutlined, FileAddOutlined, DownOutlined, UpOutlined, DeleteOutlined, ExclamationCircleFilled, DownloadOutlined, CloseOutlined, FileSearchOutlined, UserAddOutlined } from '@ant-design/icons';
...
export default function Materias() {
    ...
    const expandColumns = [
        ...
        { title: 'Profesores', dataIndex: 'teachers', key: 'teachers' },
        {
            title: ' ',
            render: (text, record) => (
                <Tooltip title="Asignar profesor">
                    <Button
                        onClick={() => {
                            showDrawer(
                                <AsignarProfesor
        ...
        },
        {
            title: ' ',
            render: (text, record) => {
                const parentRecord = materias.find(materia =>
                    materia.courses && materia.courses.some(child => child.id === record.id)
                );
                ...
                return (
                    <Tooltip title="Eliminar">
                        <Button
                            onClick={() => showDeleteConfirm(record, parentRecord)} // Pasa parentRecord a la función
                            size="default"
                            style={{ display: 'flex', justifyContent: 'center', margin: 'auto' }}
                            type="link"
                            danger
    ...
    const showDeleteConfirm = (record, parentRecord) => {
        confirm({
            title: '¿Seguro que quieres eliminar esta materia?',
            icon: <ExclamationCircleFilled />,
            content: (<span> Eliminaràs todo lo relacionado con esta materia: <b>{parentRecord?.name} - {record?.course}</b>        </span>),
            okText: 'Si',
            okType: 'danger',
            cancelText: 'Cancelar',
            onOk() {
                deleteCourseSubject(record);
                console.log('delete');
            },
            onCancel() {
                console.log('Cancel');
